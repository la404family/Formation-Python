...
print("\n" + "=" * 50)
print("9. EXERCICES AVANCÉS")
print("=" * 50)

print("""
💪 EXERCICES EXPERTS (décommentez pour tester) :

# Exercice 1 : Système de recommandation
# def systeme_recommandation():
#     # Base de données d'utilisateurs et leurs préférences
#     utilisateurs = {
#         'Alice': ['Python', 'JavaScript', 'React', 'Django'],
#         'Bob': ['Java', 'Spring', 'Python', 'SQL'],
#         'Charlie': ['JavaScript', 'Vue', 'Node.js', 'MongoDB'],
#         'Diana': ['Python', 'Flask', 'PostgreSQL', 'Docker'],
#         'Eve': ['React', 'Redux', 'Node.js', 'Express']
#     }
#     
#     def calculer_similarite(user1, user2):
#         \"\"\"Calcule la similarité entre deux utilisateurs\"\"\"
#         set1, set2 = set(utilisateurs[user1]), set(utilisateurs[user2])
#         intersection = len(set1.intersection(set2))
#         union = len(set1.union(set2))
#         return intersection / union if union > 0 else 0
#     
#     def recommander(utilisateur, n=3):
#         \"\"\"Recommande des technologies à un utilisateur\"\"\"
#         if utilisateur not in utilisateurs:
#             return []
#         
#         # Calculer similarités avec tous les autres
#         similarites = [
#             (autre, calculer_similarite(utilisateur, autre))
#             for autre in utilisateurs.keys()
#             if autre != utilisateur
#         ]
#         
#         # Trier par similarité
#         similarites.sort(key=lambda x: x[1], reverse=True)
#         
#         # Collecter technologies des utilisateurs similaires
#         technologies_recommandees = set()
#         technologies_utilisateur = set(utilisateurs[utilisateur])
#         
#         for autre, _ in similarites[:2]:  # Top 2 utilisateurs similaires
#             technologies_recommandees.update(utilisateurs[autre])
#         
#         # Exclure celles déjà connues
#         nouvelles = technologies_recommandees - technologies_utilisateur
#         
#         return list(nouvelles)[:n]
#     
#     # Test
#     for user in utilisateurs.keys():
#         recommandations = recommander(user)
#         print(f"{user}: {recommandations}")

# Exercice 2 : Analyseur de logs avec patterns
# def analyseur_logs_avance():
#     logs = [
#         "2024-01-15 10:30:15 INFO Login successful for user alice",
...
#             
#             # Extraire utilisateur si présent
#             user_pattern = r'user (\w+)'
#             user_match = re.search(user_pattern, message)
#             utilisateur = user_match.group(1) if user_match else None
#             
#             return {
#                 'timestamp': timestamp,
#                 'niveau': niveau,
...
#     
#     # Analyses
#     niveaux = Counter(log['niveau'] for log in logs_structures)
#     activite_users = defaultdict(list)
#     
#     for log in logs_structures:
#         if log['utilisateur']:
#             activite_users[log['utilisateur']].append(log)
#     
#     print("Niveaux de logs:", dict(niveaux))
#     print("Activité par utilisateur:")
#     for user, logs_user in activite_users.items():
#         print(f"  {user}: {len(logs_user)} actions")

# Exercice 3 : Optimiseur de portefeuille
# def optimiseur_portefeuille():
#     # Données financières simulées
#     actions = {
#         'AAPL': {'prix': 150, 'volatilite': 0.25, 'rendement_annuel': 0.12},
#         'GOOGL': {'prix': 2800, 'volatilite': 0.30, 'rendement_annuel': 0.15},
#         'MSFT': {'prix': 300, 'volatilite': 0.22, 'rendement_annuel': 0.11},
#         'TSLA': {'prix': 200, 'volatilite': 0.45, 'rendement_annuel': 0.20},
#         'AMZN': {'prix': 3200, 'volatilite': 0.35, 'rendement_annuel': 0.14}
#     }
#     
...
#     
#     def optimiser_portefeuille(budget, max_actions_par_titre=10):
#         \"\"\"Optimise un portefeuille selon le ratio de Sharpe\"\"\"
#         
#         # Calculer ratios de Sharpe
#         ratios = {}
#         for symbole, donnees in actions.items():
#             ratio = calculer_sharpe_ratio(
#                 donnees['rendement_annuel'], 
#                 donnees['volatilite']
#             )
#             ratios[symbole] = ratio
#         
...
#         
#         for symbole, ratio in actions_triees:
#             prix = actions[symbole]['prix']
#             max_possible = min(max_actions_par_titre, budget_restant // prix)
#             
#             if max_possible > 0:
#                 portefeuille[symbole] = max_possible
#                 budget_restant -= max_possible * prix
#         
#         return portefeuille, budget_restant
#     
#     # Test avec 50000€
#     portefeuille_optimal, reste = optimiser_portefeuille(50000)
#     
#     print("Portefeuille optimal pour 50000€:")
#     valeur_totale = 0
#     for symbole, quantite in portefeuille_optimal.items():
#         valeur = quantite * actions[symbole]['prix']
#         valeur_totale += valeur
#         ratio = calculer_sharpe_ratio(
#             actions[symbole]['rendement_annuel'],
#             actions[symbole]['volatilite']
#         )
#         print(f"  {symbole}: {quantite} actions = {valeur}€ (Sharpe: {ratio:.3f})")
#     
#     print(f"Valeur investie: {valeur_totale}€")
#     print(f"Liquidités: {reste}€")

# Exercice 4 : Moteur de règles métier
# def moteur_regles():
#     class RegleMetier:
#         def __init__(self, nom, condition, action):
#             self.nom = nom
#             self.condition = condition
#             self.action = action
#         
#         def evaluer(self, contexte):
#             if self.condition(contexte):
#                 return self.action(contexte)
#             return None
#     
...
#         RegleMetier(
#             "Remise fidélité",
#             lambda ctx: ctx.get('achats_precedents', 0) > 10,
#             lambda ctx: ctx.get('prix_base', 0) * 0.9
#         ),
#         RegleMetier(
#             "Remise volume",
#             lambda ctx: ctx.get('quantite', 0) > 100,
#             lambda ctx: ctx.get('prix_base', 0) * 0.85
#         ),
#         RegleMetier(
#             "Prix premium weekend",
#             lambda ctx: ctx.get('jour_semaine', '') in ['samedi', 'dimanche'],
#             lambda ctx: ctx.get('prix_base', 0) * 1.15
#         ),
#         RegleMetier(
#             "Prix standard",
#             lambda ctx: True,  # Règle par défaut
#             lambda ctx: ctx.get('prix_base', 0)
#         )
#     ]
#     
#     def calculer_prix(contexte):
#         for regle in regles_pricing:
#             prix = regle.evaluer(contexte)
#             if prix is not None:
#                 return prix, regle.nom
#         return contexte.get('prix_base', 0), "Aucune règle"
#     
#     # Tests
#     scenarios = [
#         {'prix_base': 100, 'achats_precedents': 15, 'quantite': 5, 'jour_semaine': 'lundi'},
#         {'prix_base': 100, 'achats_precedents': 2, 'quantite': 150, 'jour_semaine': 'mardi'},
#         {'prix_base': 100, 'achats_precedents': 3, 'quantite': 10, 'jour_semaine': 'samedi'},
#         {'prix_base': 100, 'achats_precedents': 5, 'quantite': 20, 'jour_semaine': 'mercredi'}
#     ]
#     
#     for i, scenario in enumerate(scenarios, 1):
#         prix, regle = calculer_prix(scenario)
#         print(f"Scénario {i}: {prix}€ (règle: {regle})")
""")

print("\n" + "=" * 50)
print("10. RÉSUMÉ ET BONNES PRATIQUES")
print("=" * 50)
