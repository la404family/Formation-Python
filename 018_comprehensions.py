...
# Exercice 1 : Analyseur de structure de projet
# import os
# 
# def analyser_structure_projet(chemin_racine):
#     \"\"\"Analyse la structure d'un projet avec compréhensions\"\"\"
#     
#     # Simuler une structure de fichiers
#     fichiers_simules = [
#         "src/main.py", "src/utils.py", "src/models/user.py",
#         "tests/test_main.py", "tests/test_utils.py",
#         "docs/readme.md", "docs/api.md",
#         "config/settings.json", "requirements.txt"
#     ]
#     
#     # Analyse par extension
#     extensions = {
#         fichier.split('.')[-1]: fichier.split('.')[-1]
#         for fichier in fichiers_simules
#     }
#     
#     # Groupement par dossier
#     par_dossier = {}
#     for fichier in fichiers_simules:
#         dossier = '/'.join(fichier.split('/')[:-1]) or 'racine'
#         if dossier not in par_dossier:
#             par_dossier[dossier] = []
#         par_dossier[dossier].append(fichier.split('/')[-1])
#     
#     # Statistiques avec compréhensions
#     stats = {
#         'total_fichiers': len(fichiers_simules),
#         'fichiers_python': len([f for f in fichiers_simules if f.endswith('.py')]),
#         'fichiers_config': len([f for f in fichiers_simules if f.endswith(('.json', '.yml', '.yaml'))]),
#         'fichiers_doc': len([f for f in fichiers_simules if f.endswith(('.md', '.txt'))]),
#         'dossiers': list(par_dossier.keys()),
#         'extensions_uniques': list(set(f.split('.')[-1] for f in fichiers_simules))
#     }
#     
#     return stats, par_dossier
# 
# # Test
# stats, structure = analyser_structure_projet("./projet")
...
# import string
# 
# def generer_mots_de_passe(nb_mots=5, longueur=12):
#     \"\"\"Génère des mots de passe avec différents critères\"\"\"
#     
#     caracteres = {
...
#     }
#     
#     # Mots de passe avec compréhensions
#     mots_de_passe = [
#         ''.join([
#             random.choice(caracteres['majuscules']),  # Au moins 1 majuscule
#             random.choice(caracteres['chiffres']),    # Au moins 1 chiffre
#             random.choice(caracteres['speciaux']),    # Au moins 1 spécial
#             *[random.choice(''.join(caracteres.values())) 
#               for _ in range(longueur - 3)]           # Le reste aléatoire
#         ])
#         for _ in range(nb_mots)
#     ]
#     
#     # Mélanger chaque mot de passe
#     mots_melanges = [
#         ''.join(random.sample(list(mdp), len(mdp)))
#         for mdp in mots_de_passe
#     ]
#     
#     # Validation avec compréhensions
#     validations = [
#         {
#             'mot_de_passe': mdp,
#             'longueur_ok': len(mdp) >= longueur,
#             'a_majuscule': any(c.isupper() for c in mdp),
#             'a_chiffre': any(c.isdigit() for c in mdp),
#             'a_special': any(c in caracteres['speciaux'] for c in mdp)
#         }
#         for mdp in mots_melanges
#     ]
#     
#     return validations
# 
# # Test
# mots_generes = generer_mots_de_passe(3, 10)
# for validation in mots_generes:
#     print(f"Mot de passe: {validation['mot_de_passe']}")
#     print(f"  Validations: {[(k, v) for k, v in validation.items() if k != 'mot_de_passe']}")

# Exercice 3 : Analyseur de performances d'équipe
# def analyser_performances_equipe():
#     # Données de performance simulées
#     performances = [
#         {'nom': 'Alice', 'projets': [85, 92, 78, 88], 'equipe': 'A'},
#         {'nom': 'Bob', 'projets': [79, 85, 83, 87], 'equipe': 'A'},
#         {'nom': 'Charlie', 'projets': [95, 89, 92, 90], 'equipe': 'B'},
#         {'nom': 'Diana', 'projets': [88, 91, 85, 89], 'equipe': 'B'},
#         {'nom': 'Eve', 'projets': [82, 84, 80, 86], 'equipe': 'A'},
#         {'nom': 'Frank', 'projets': [90, 88, 91, 89], 'equipe': 'B'}
#     ]
#     
#     # Calculs avec compréhensions
#     moyennes_individuelles = [
#         {
#             'nom': p['nom'],
#             'moyenne': sum(p['projets']) / len(p['projets']),
#             'equipe': p['equipe'],
#             'progression': p['projets'][-1] - p['projets'][0],  # Dernier - Premier projet
#             'constance': max(p['projets']) - min(p['projets'])  # Écart max-min
#         }
#         for p in performances
#     ]
#     
#     # Top performers avec compréhensions
#     top_performers = [
#         p for p in moyennes_individuelles 
#         if p['moyenne'] > 87 and p['constance'] < 10
#     ]
#     
#     # Moyennes par équipe
#     equipes = {p['equipe'] for p in performances}
#     moyennes_equipes = {
#         equipe: {
#             'moyenne_equipe': sum(
#                 sum(p['projets']) / len(p['projets'])
#                 for p in performances if p['equipe'] == equipe
#             ) / len([p for p in performances if p['equipe'] == equipe]),
#             'membres': [p['nom'] for p in performances if p['equipe'] == equipe],
#             'progression_equipe': sum(
#                 p['projets'][-1] - p['projets'][0]
#                 for p in performances if p['equipe'] == equipe
#             )
#         }
...
#     
#     return {
#         'individuelles': moyennes_individuelles,
#         'top_performers': top_performers,
#         'equipes': moyennes_equipes
#     }
# 
# # Test
# analyse = analyser_performances_equipe()
# print("Top performers:")
# for tp in analyse['top_performers']:
#     print(f"  {tp['nom']}: {tp['moyenne']:.1f} (progression: {tp['progression']:+d})")

# Exercice 4 : Optimiseur de planning
# def optimiser_planning():
#     # Créneaux et contraintes
#     creneaux = [f"{h:02d}:00-{h+1:02d}:00" for h in range(9, 18)]  # 9h à 17h
#     jours = ['Lundi', 'Mardi', 'Mercredi', 'Jeudi', 'Vendredi']
#     
#     # Activités avec durées et priorités
#     activites = [
#         {'nom': 'Réunion équipe', 'duree': 2, 'priorite': 8, 'type': 'reunion'},
#         {'nom': 'Développement', 'duree': 4, 'priorite': 9, 'type': 'dev'},
#         {'nom': 'Tests', 'duree': 2, 'priorite': 7, 'type': 'test'},
#         {'nom': 'Documentation', 'duree': 1, 'priorite': 5, 'type': 'doc'},
#         {'nom': 'Formation', 'duree': 3, 'priorite': 6, 'type': 'formation'},
#         {'nom': 'Code Review', 'duree': 1, 'priorite': 8, 'type': 'review'}
#     ]
#     
#     # Génération de plannings possibles avec compréhensions
#     # Simplification : on assigne les activités dans l'ordre de priorité
#     activites_triees = sorted(activites, key=lambda x: x['priorite'], reverse=True)
#     
#     planning_optimal = []
#     creneaux_utilises = 0
#     max_creneaux_jour = len(creneaux)
#     jour_actuel = 0
#     
#     for activite in activites_triees:
#         if creneaux_utilises + activite['duree'] <= max_creneaux_jour:
#             planning_optimal.append({
#                 'jour': jours[jour_actuel],
#                 'activite': activite['nom'],
#                 'heure_debut': creneaux[creneaux_utilises],
#                 'duree': activite['duree'],
#                 'priorite': activite['priorite']
#             })
#             creneaux_utilises += activite['duree']
#         else:
#             # Passer au jour suivant si possible
#             if jour_actuel < len(jours) - 1:
#                 jour_actuel += 1
#                 creneaux_utilises = 0
#                 planning_optimal.append({
#                     'jour': jours[jour_actuel],
...
#                 creneaux_utilises += activite['duree']
#     
#     # Statistiques du planning avec compréhensions
#     stats_planning = {
#         'total_activites': len(planning_optimal),
#         'charge_par_jour': {
#             jour: sum(p['duree'] for p in planning_optimal if p['jour'] == jour)
#             for jour in jours
#         },
#         'priorite_moyenne': sum(p['priorite'] for p in planning_optimal) / len(planning_optimal),
#         'types_activites': {
#             type_act: len([a for a in activites if a['type'] == type_act])
#             for type_act in {a['type'] for a in activites}
#         }
#     }
#     
#     return planning_optimal, stats_planning
# 
# # Test
# planning, stats = optimiser_planning()
# print("Planning optimisé:")
# for p in planning:
#     print(f"  {p['jour']} {p['heure_debut']}: {p['activite']} ({p['duree']}h, priorité {p['priorite']})")
# print("\\nStatistiques:", stats)
""")
