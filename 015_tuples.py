...
# Exercice 1 : Gestionnaire de versions
# Version = namedtuple('Version', ['majeur', 'mineur', 'patch'])
# 
# def comparer_versions(v1, v2):
#     \"\"\"Compare deux versions (retourne -1, 0, ou 1)\"\"\"
#     if v1 < v2:
#         return -1
#     elif v1 > v2:
#         return 1
...
#         return 0
# 
# def incrementer_version(version, type_increment='patch'):
#     \"\"\"Incrémente une version\"\"\"
#     if type_increment == 'majeur':
#         return version._replace(majeur=version.majeur + 1, mineur=0, patch=0)
#     elif type_increment == 'mineur':
#         return version._replace(mineur=version.mineur + 1, patch=0)
#     else:  # patch
#         return version._replace(patch=version.patch + 1)
# 
# # Tests
# v1 = Version(1, 2, 3)
# v2 = Version(1, 3, 0)
...
# print(f"v1 vs v2 : {comparer_versions(v1, v2)}")
# print(f"v1 patch+ : {incrementer_version(v1, 'patch')}")
# print(f"v1 mineur+ : {incrementer_version(v1, 'mineur')}")

# Exercice 2 : Cache avec tuples comme clés
# class CacheCalcul:
#     def __init__(self):
#         self.cache = {}
#     
#     def calculer_cout(self, distance, poids, priorite):
#         \"\"\"Calcule le coût d'expédition avec cache\"\"\"
#         cle = (distance, poids, priorite)
#         
...
#         cout_base = distance * 0.5
#         cout_poids = poids * 0.3
#         multiplicateur_priorite = 1.5 if priorite == 'urgent' else 1.0
#         
#         cout_total = (cout_base + cout_poids) * multiplicateur_priorite
#         
#         self.cache[cle] = cout_total
#         print(f"Calcul pour {cle} : {cout_total:.2f}€")
#         return cout_total
#     
#     def statistiques_cache(self):
#         return len(self.cache)
# 
# # Test
# cache = CacheCalcul()
# cache.calculer_cout(100, 5, 'normal')
# cache.calculer_cout(200, 3, 'urgent')
# cache.calculer_cout(100, 5, 'normal')  # Cache hit
# print(f"Entrées en cache : {cache.statistiques_cache()}")

# Exercice 3 : Analyse de trajectoire
# Point3D = namedtuple('Point3D', ['x', 'y', 'z', 'temps'])
# 
# def analyser_trajectoire(points):
#     \"\"\"Analyse une trajectoire de points 3D\"\"\"
#     if len(points) < 2:
#         return None
#     
...
#         vitesse = distance / dt if dt > 0 else 0
...
#         'duree_totale': points[-1].temps - points[0].temps
#     }
# 
# # Test avec une trajectoire simulée
# trajectoire = [
#     Point3D(0, 0, 0, 0),
#     Point3D(10, 0, 0, 1),
#     Point3D(10, 10, 0, 2),
...
# ]
# 
# analyse = analyser_trajectoire(trajectoire)
# if analyse:
#     print("Analyse de trajectoire :")
#     for cle, valeur in analyse.items():
#         print(f"  {cle}: {valeur:.2f}")

# Exercice 4 : Système de notation RGB
# def rgb_vers_hex(rouge, vert, bleu):
#     \"\"\"Convertit RGB en hexadécimal\"\"\"
#     return f"#{rouge:02x}{vert:02x}{bleu:02x}"
# 
# def hex_vers_rgb(hex_color):
#     \"\"\"Convertit hexadécimal en RGB tuple\"\"\"
#     hex_color = hex_color.lstrip('#')
#     return tuple(int(hex_color[i:i+2], 16) for i in (0, 2, 4))
# 
# def melanger_couleurs(*couleurs):
#     \"\"\"Mélange plusieurs couleurs RGB\"\"\"
#     if not couleurs:
#         return (0, 0, 0)
#     
#     rouge_total = sum(c[0] for c in couleurs) // len(couleurs)
#     vert_total = sum(c[1] for c in couleurs) // len(couleurs)
#     bleu_total = sum(c[2] for c in couleurs) // len(couleurs)
#     
#     return (rouge_total, vert_total, bleu_total)
# 
# # Tests
# rouge = (255, 0, 0)
# vert = (0, 255, 0)
# bleu = (0, 0, 255)
# 
# print(f"Rouge en hex : {rgb_vers_hex(*rouge)}")
# print(f"#00ff00 en RGB : {hex_vers_rgb('#00ff00')}")
# 
# violet = melanger_couleurs(rouge, bleu)
# print(f"Mélange rouge + bleu : {violet}")
# print(f"Violet en hex : {rgb_vers_hex(*violet)}")

# Exercice 5 : Base de données de films
# Film = namedtuple('Film', ['titre', 'realisateur', 'annee', 'genre', 'note'])
# 
# def analyser_collection_films(films):
#     \"\"\"Analyse une collection de films\"\"\"
#     
#     # Groupement par décennie
#     par_decennie = {}
#     for film in films:
#         decennie = (film.annee // 10) * 10
#         if decennie not in par_decennie:
#             par_decennie[decennie] = []
#         par_decennie[decennie].append(film)
#     
#     # Meilleurs films par genre
#     par_genre = {}
#     for film in films:
#         if film.genre not in par_genre:
#             par_genre[film.genre] = []
#         par_genre[film.genre].append(film)
#     
#     meilleurs_par_genre = {}
#     for genre, liste_films in par_genre.items():
#         meilleur = max(liste_films, key=lambda f: f.note)
#         meilleurs_par_genre[genre] = meilleur
#     
#     return {
#         'par_decennie': par_decennie,
#         'meilleurs_par_genre': meilleurs_par_genre,
#         'note_moyenne': sum(f.note for f in films) / len(films)
#     }
# 
# # Collection test
# collection = [
#     Film("Pulp Fiction", "Tarantino", 1994, "Crime", 8.9),
#     Film("The Matrix", "Wachowski", 1999, "Sci-Fi", 8.7),
#     Film("Goodfellas", "Scorsese", 1990, "Crime", 8.7),
#     Film("Blade Runner", "Scott", 1982, "Sci-Fi", 8.1),
#     Film("Casablanca", "Curtiz", 1942, "Romance", 8.5)
# ]
# 
# analyse = analyser_collection_films(collection)
# print(f"Note moyenne : {analyse['note_moyenne']:.2f}")
# print("Meilleurs par genre :")
# for genre, film in analyse['meilleurs_par_genre'].items():
#     print(f"  {genre}: {film.titre} ({film.note})")
""")
